...
export default function TakeQuizPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSelectAnswer = (answerIndex: number) => {
    const existingAnswer = answers.find(a => a.questionIndex === currentQuestion)
    if (existingAnswer) {
      ...
      setAnswers([...answers, { questionIndex: currentQuestion, selectedAnswer: answerIndex }])
    }
  }
  ...
  if (result) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#205072] via-[#329D9C] to-[#56C596] flex items-center justify-center p-4">
              ...
              {question.options.map((option, idx) => (
                <div
                  ...
                  className={`p-4 rounded-xl border-2 cursor-pointer transition-all ${
                    getSelectedAnswer() === idx
                      ? "border-[#329D9C] bg-[#329D9C]/10"
                      : "border-slate-200 hover:border-[#329D9C]/50"
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${
                      getSelectedAnswer() === idx
                        ? "border-[#329D9C] bg-[#329D9C]"
                        : "border-slate-300"
                    }`}>
                      {getSelectedAnswer() === idx && (
                    ...
                    <span className="text-[#205072]">{option}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="flex items-center justify-between mt-6">
          ...
          {currentQuestion === quiz.questions.length - 1 ? (
            ...
            <Button
              ...
              className="gap-2 bg-white text-[#205072] hover:bg-white/90"
            >
              Next
            ...
            <button
              ...
              className={`w-8 h-8 rounded-lg text-sm font-medium transition-all ${
                idx === currentQuestion
                  ...
                  : answers.find(a => a.questionIndex === idx)
